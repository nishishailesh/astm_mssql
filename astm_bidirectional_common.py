...
class ms_sql(object):
  def get_link(self,my_host,my_user,my_pass,my_db):
    try:
      con=pyodbc.connect('DRIVER=FreeTDS;SERVER='+my_host+';PORT=1433;DATABASE='+my_db+';UID='+my_user+';PWD='+my_pass+';TDS_Version=8.0;Encrypt=yes')
      con.autocommit = True #not in MySQLdb
    except Exception as my_ex:
      msg="can not connect to remote database (internet problem? database server down?.."
      print_to_log(my_ex,msg)
      return None
    logging.debug(con)
    if(con==None):
      print_to_log("Can't connect to database",'internet problem?')
      return None
    else:
      pass
  ...
  def run_query(self,con,prepared_sql,data_tpl=tuple()):
    cur=con.cursor()
    cur.execute(prepared_sql,data_tpl)
    #con.commit() #only in MySQLdb , in mssql giving Function sequence error (0)
